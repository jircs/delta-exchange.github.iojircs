...
| showWhenLogin | Show the card only when user is logged in or logged out state or both. If not specified then card is shown in all the states. | "preLogin" or "postLogin" | "showWhenLogin" : "preLogin" | Only Image Card | Optional |
...
| showWhenLogin | Show the card only when user is logged in or logged out state or both. If not specified then card is shown in all the states. | "preLogin" or "postLogin" | "showWhenLogin" : "preLogin" | Only Image Card | Optional |
| showWhenMobile | Show the card only when user is in mobile browser or mobile app or both. If not specified then card is shown in all the states | "browser" or "app" | "showWhenLogin" : "app" | Only Image Card | Optional |
